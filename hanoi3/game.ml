...
(* 軸の定義 *)
let bars = [
  (rectangle 100. 20. sandyBrown); (rectangle 20. 200. sandyBrown);
  (rectangle 100. 20. sandyBrown); (rectangle 20. 200. sandyBrown);
  (rectangle 100. 20. sandyBrown); (rectangle 20. 200. sandyBrown);
]
...
let end_with (world : world_t) : world_t =
  ...
  match fetch_bar_info world 3 with {height = h3} ->
    ...
    if h3 = 3. then {world with remaining = m - total_moved;
                                    msg = "; )"}
    else
      begin
        if total_moved = 10                 (* 既定回数に到達 *)
        then {world with remaining = m - total_moved;
                         msg = "try again"}
        else if total_moved > 10 then world (* 既定回数以上は操作回数が減らない *)
